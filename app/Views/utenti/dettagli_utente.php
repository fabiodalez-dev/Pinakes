...
<div class="space-y-6">
  ...
  <?php if ($success === 'approved_email_sent'): ?>
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3" role="alert">
      ...
      <div>
        <p class="font-medium text-blue-900">Utente approvato con successo!</p>
        <p class="text-sm text-blue-700 mt-1">L'email di attivazione è stata inviata. L'utente potrà verificare il proprio account cliccando il link ricevuto (valido 7 giorni).</p>
      </div>
    </div>
  <?php endif; ?>
  ...
  <?php if ($success === 'activated_directly'): ?>
    <div class="bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3" role="alert">
      ...
      <div>
        <p class="font-medium text-green-900">Utente attivato direttamente!</p>
        <p class="text-sm text-green-700 mt-1">L'utente è stato attivato e può già effettuare il login. È stata inviata un'email di benvenuto.</p>
      </div>
    </div>
  <?php endif; ?>
  ...
  <?php if ($error === 'user_not_found'): ?>
    <div class="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3" role="alert">
      ...
      <div>
        <p class="font-medium text-red-900">Errore: Utente non trovato</p>
        <p class="text-sm text-red-700 mt-1">L'utente richiesto non esiste nel database.</p>
      </div>
    </div>
  <?php endif; ?>
  ...
  <?php if ($error === 'not_suspended'): ?>
    <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
      ...
      <div>
        <p class="font-medium text-amber-900">Operazione non consentita</p>
        <p class="text-sm text-amber-700 mt-1">L'utente non è in stato sospeso. Solo gli utenti sospesi richiedono approvazione.</p>
      </div>
    </div>
  <?php endif; ?>
  ...
  <?php if ($error === 'db_error'): ?>
    <div class="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3" role="alert">
      ...
      <div>
        <p class="font-medium text-red-900">Errore del database</p>
        <p class="text-sm text-red-700 mt-1">Si è verificato un errore durante l'operazione. Riprova più tardi.</p>
      </div>
    </div>
  <?php endif; ?>
  ...
  <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
    <div>
      ...
      <h1 class="mt-2 text-2xl font-bold text-gray-900"><?= $display($name, 'Utente senza nome'); ?></h1>
      <p class="text-sm text-gray-500">Ruolo: <?= $display($roleLabels[$ruolo] ?? ucfirst($ruolo)); ?></p>
    </div>
  ...
  <section class="bg-white border border-gray-200 rounded-lg p-6">
    ...
    <dl class="mt-4 grid gap-4 sm:grid-cols-2">
      <div>
        <dt class="text-sm text-gray-500">Nome completo</dt>
      ...
      <div>
        <dt class="text-sm text-gray-500">Sesso</dt>
  ...
  <section class="bg-white border border-gray-200 rounded-lg p-6">
    ...
    <dl class="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <div>
        <dt class="text-sm text-gray-500">Codice Tessera</dt>
        <dd class="text-sm text-gray-900 mt-1 font-mono"><?= $display($codiceTessera); ?></dd>
      </div>
      <div>
        <dt class="text-sm text-gray-500">Registrato il</dt>
        <dd class="text-sm text-gray-900 mt-1">
          <?= !empty($creatoIl) ? date('d/m/Y H:i', strtotime($creatoIl)) : '—'; ?>
        </dd>
      </div>
      <div>
        <dt class="text-sm text-gray-500">Ultimo aggiornamento</dt>
        <dd class="text-sm text-gray-900 mt-1">
          <?= !empty($aggiornatoIl) ? date('d/m/Y H:i', strtotime($aggiornatoIl)) : '—'; ?>
        </dd>
      </div>
      <div>
        <dt class="text-sm text-gray-500">Scadenza tessera</dt>
        <dd class="text-sm text-gray-900 mt-1">
          <?= !empty($dataScadenzaTessera) ? date('d/m/Y', strtotime($dataScadenzaTessera)) : '—'; ?>
        </dd>
      </div>
      <div>
        <dt class="text-sm text-gray-500">Ultimo accesso</dt>
        <dd class="text-sm text-gray-900 mt-1">
  ...
  <section class="bg-amber-50 border border-amber-200 rounded-lg p-6">
    <div class="flex items-center gap-2 mb-4">
      ...
      <h2 class="text-lg font-medium text-gray-900">Azioni di Approvazione</h2>
    </div>

    <div class="mb-4 p-4 bg-white border border-amber-200 rounded-lg">
      <p class="text-sm text-gray-700">
        <i class="fas fa-info-circle text-amber-600 mr-2"></i>
        Questo utente è in stato <strong>sospeso</strong> e richiede approvazione. Scegli un'opzione:
      </p>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      <form method="POST" action="/admin/utenti/<?= $id; ?>/approve-and-send-activation" class="flex-1">
        ...
        <button type="submit" class="w-full px-4 py-3 bg-gray-900 text-white hover:bg-blue-700 rounded-lg transition-colors duration-200 inline-flex items-center justify-center gap-2 border border-blue-700">
          ...
          <span class="font-medium">Approva e Invia Email Attivazione</span>
        </button>
        <p class="text-xs text-gray-600 mt-2">
          L'utente riceverà un'email con link di verifica (valido 7 giorni) e potrà attivare autonomamente l'account.
        </p>
      </form>
      ...
      <form method="POST" action="/admin/utenti/<?= $id; ?>/activate-directly" class="flex-1" onsubmit="return confirm('<?= addslashes(__('Confermi di voler attivare direttamente questo utente senza richiedere verifica email?')) ?>')">
        ...
        <button type="submit" class="w-full px-4 py-3 bg-green-600 text-white hover:bg-green-700 rounded-lg transition-colors duration-200 inline-flex items-center justify-center gap-2 border border-green-700">
          ...
          <span class="font-medium">Attiva Direttamente</span>
        </button>
        <p class="text-xs text-gray-600 mt-2">
          L'utente sarà attivato immediatamente e riceverà un'email di benvenuto. Potrà accedere subito.
        </p>
      </form>
    </div>
  </section>
  ...
  <div class="bg-white shadow-sm rounded-2xl border border-slate-200">
    <div class="px-6 py-4 border-b border-slate-200">
        <h2 class="text-lg font-semibold text-gray-800">Storico Prestiti</h2>
    </div>
                ...
                <?php if (empty($prestiti)): ?>
                    <tr>
                        <td colspan="4" class="text-center py-10 text-gray-500">
                            ...
                            <p>Questo utente non ha mai effettuato prestiti.</p>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($prestiti as $prestito): ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-gray-500">ID Prestito: <?= $prestito['id']; ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700">
                                <div>
                                    <span class="font-semibold">Dal:</span> <?= date("d/m/Y", strtotime($prestito['data_prestito'])); ?>
                                </div>
                                <div>
                                    <span class="font-semibold">Al:</span> <?= !empty($prestito['data_restituzione']) ? date("d/m/Y", strtotime($prestito['data_restituzione'])) : date("d/m/Y", strtotime($prestito['data_scadenza'])); ?>
                                </div>
                            </td>
  ...
  <?php if ($note !== ''): ?>
    <section class="bg-white border border-gray-200 rounded-lg p-6 mt-6">
      <h2 class="text-lg font-medium text-gray-900">Note interne</h2>
