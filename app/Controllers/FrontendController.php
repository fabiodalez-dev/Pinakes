...
class FrontendController
{
    public function home(Request $request, Response $response, mysqli $db): Response
    {
        ...
        // Ottieni i generi più popolari (quelli con più libri)
        $query_popular_genres = "
            SELECT g.id, g.nome, COUNT(l.id) as libro_count
            FROM generi g
            JOIN libri l ON g.id = l.genere_id
            GROUP BY g.id, g.nome
            HAVING libro_count > 0
            ORDER BY libro_count DESC, g.nome ASC
            LIMIT 6
        ";
        $result_genres = $db->query($query_popular_genres);
        $genres_with_books = [];
        ...
        if ($result_genres) {
            while ($genre = $result_genres->fetch_assoc()) {
                $query_genre_books = "
                    ...
                    LIMIT 4
                ";
                $stmt_genre = $db->prepare($query_genre_books);
                $stmt_genre->bind_param("i", $genre['id']);
                $stmt_genre->execute();
                $result_genre_books = $stmt_genre->get_result();
