...
## Integrazione con Google Books

Se disponi di una Google Books API Key puoi abilitare un ulteriore livello di arricchimento:

1. Vai su **Amministrazione → Plugin → Open Library**.
2. Inserisci la chiave nel box **Google Books API** e salva.
3. Il plugin interrogherà prima Google Books, poi Open Library (Scraping Pro mantiene comunque la priorità massima).

Quando la chiave è configurata:

- i dati di Google Books vengono restituiti se disponibili (titolo, descrizione, categorie, copertine ad alta qualità);
- in assenza di risultati, il plugin torna automaticamente su Open Library;
- le copertine mancanti possono essere recuperate da Google Books come fallback durante l'hook `scrape.data.modify`.
