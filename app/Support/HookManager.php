...
class HookManager
{
            ...
            try {
                if (isset($hook['callback'])) {
                    ...
                    if (!class_exists($className)) {
                        $classFile = $this->resolvePluginWrapper($pluginPath);
                        if ($classFile) {
                            require_once $classFile;
                        } else {
                            error_log("[HookManager] Plugin path non valido per hook {$hookName}: {$pluginPath}");
                            continue;
                        }
                    }

                    if (class_exists($className)) {
                        // Instantiate plugin with dependencies (db, hookManager)
                        $instance = new $className($this->db, $this);
                        $method = $hook['method'];

                        // Use is_callable() instead of method_exists() to support __call() magic methods
    ...
    /**
     * Resolve a safe wrapper path for a plugin before loading arbitrary code
     */
    private function resolvePluginWrapper(?string $pluginPath): ?string
    {
        if (!$pluginPath || !preg_match('/^[a-z0-9_\-]+$/i', $pluginPath)) {
            return null;
        }

        $base = realpath(__DIR__ . '/../../storage/plugins');
        if ($base === false) {
            return null;
        }

        $fullPath = $base . DIRECTORY_SEPARATOR . $pluginPath . DIRECTORY_SEPARATOR . 'wrapper.php';
        ...
        if (strpos($realFullPath, $normalizedBase) !== 0) {
            return null;
        }
