...
- `GET /catalog/search` — filters: `q`, `author`, `publisher`, `genre` (cascade id), `language`, `available` (bool); `sort`: `relevance` (default when `q` is present), `newest` (default without `q`), `oldest`, `title_asc`, `title_desc`, `author_asc`, `author_desc`; cursor pagination.
