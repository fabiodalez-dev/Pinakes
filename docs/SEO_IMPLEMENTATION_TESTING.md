...
**Hreflang: intentionally NOT emitted (since 0.7.65).**

Earlier revisions of this document described path-prefixed alternates
(`/en/...`, `/de/...`) as implemented. That was never true at the routing
layer: translated route variants are registered at the **root** (`/catalogo`
AND `/catalog` both resolve) and the response language follows the **session**
locale (`/language/{locale}`), not the URL. The emitted alternates therefore
pointed to 404s, and the sitemap multiplied every entry per locale with the
same broken prefixes.

Since 0.7.65 `App\Support\HreflangHelper::getAlternates()` returns an empty
list and `SitemapGenerator` emits **only default-locale URLs**. hreflang is
only meaningful when each language variant lives at its own URL and the
content follows that URL; if locale path-prefix routing is ever implemented
(a middleware consuming `/xx` and fixing the request locale), restore the URL
mapping from git history on top of it.
...
| `GET /robots.txt` | Robots policy | Disallows `/admin/` and **every active locale's** `login`/`register` route variant (all variants are registered at the root); advertises the sitemap. Adds an `llms.txt:` line only when `seo.llms_txt_enabled = '1'`. Auth pages are correspondingly excluded from the sitemap. |
