...
- `docs/PLUGIN_HOOKS.md` - Riferimento di tutti i 53+ hook disponibili (28 implementati + 25 documentati)
...
**Esempio Avanzato - Plugin Amazon Scraper:**
```php
class AmazonScraperPlugin
{
    public function onActivate(): void {
        // Add Amazon as scraping source
        \App\Support\Hooks::add('scrape.sources', [$this, 'addAmazonSource'], 10);

        // Parse Amazon HTML
        \App\Support\Hooks::add('scrape.parse', [$this, 'parseAmazon'], 8);

        // Enrich with Amazon rating
        \App\Support\Hooks::add('scrape.data.modify', [$this, 'enrichWithRating'], 10);
    }

    public function addAmazonSource($sources, $isbn) {
        $sources['amazon'] = [
            'name' => 'Amazon',
            'url_pattern' => 'https://www.amazon.it/s?k={isbn}',
            'enabled' => true,
            'priority' => 15,
            'fields' => ['price', 'description', 'rating']
        ];
        return $sources;
    }

    public function parseAmazon($parsed, $raw, $source, $isbn) {
        if ($source['name'] !== 'Amazon') return $parsed;

        // Custom Amazon parser with XPath
        $dom = new \DOMDocument();
        @$dom->loadHTML($raw);
        $xpath = new \DOMXPath($dom);

        $parsed['price'] = $this->extractAmazonPrice($xpath);
        $parsed['rating'] = $this->extractAmazonRating($xpath);

        return $parsed;
    }

    public function enrichWithRating($payload, $isbn, $source) {
        if (isset($payload['rating'])) {
            $payload['amazon_verified'] = true;
        }
        return $payload;
    }
}
...
#### Hook Disponibili (28+ implementati)
...
**Scraping (12 hook per estensibilità):**
- `scrape.isbn.validate` - Validazione ISBN personalizzata (API esterne, DB)
- `scrape.sources` - Aggiungi fonti scraping (Amazon, IBS, API custom)
- `scrape.fetch.custom` - Sostituisci completamente logica scraping
- `scrape.before` / `scrape.after` - Pre/post processing (cache, logging)
- `scrape.http.options` - Customizza opzioni curl (headers, timeout, proxy)
- `scrape.parse` - Parser personalizzati per fonti custom
- `scrape.validate.data` - Validazione dati scrapati
- `scrape.validation.failed` - Gestione errori validazione
- `scrape.data.modify` - Arricchimento dati (Goodreads rating, normalizzazione)
- `scrape.error` - Gestione errori (logging, Sentry, notifiche)
- `scrape.response` - Modifica JSON response finale
