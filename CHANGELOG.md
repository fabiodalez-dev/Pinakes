...
### Fixed
- **Coming back to the site with "remember me" no longer breaks the first form you submit.** A browser returning with only the remember-me cookie asks for several things at once — the page, and whatever that page fetches for itself. Every one of those requests arrived without a session, every one of them authenticated from the same cookie, and every one of them created its own session with its own CSRF token. The browser keeps the last one it is given, so the token printed into the page could belong to a session that had already been left behind, and the next thing the visitor submitted was refused with "Errore di Sicurezza" — for no reason they could see, and not reliably enough to report. Measured on the admin dashboard, whose own statistics request is the usual second creator: it happened on about half of page loads. The first request to authenticate a token now records the session it made, and a sibling arriving in the next few seconds joins it instead of starting another; the note is published in one step and only where there is none already, so two requests reaching that point at the same instant still agree on one session rather than publishing an id each. The note is keyed by a hash of the token, never holds the token, is dropped whenever the cookie is retired, and is swept once it is past its short window.
...
- **A facet list that is cut off now says so.** Reported from a live library: in the catalogue sidebar the authors list stopped mid-alphabet with ninety-two names still inside it, and nothing said it could be scrolled — no rule closing the box, no fade, and a thin scrollbar most platforms only paint while the pointer is over it. It read as a finished list, which in turn made the short facet below it look broken rather than short. Every open facet list is now closed by a hairline, and while there is content below the fold its last line fades out; the fade clears once the list is scrolled to the bottom, because a cue that never goes away stops being read as one. It is a mask rather than a gradient over the content on purpose: the panel is transparent and the four layouts and the colour theme paint different things behind it, so a fade hard-coded to white would be a white smear on three of them. On a phone the filters panel starts closed, and a closed panel has no height to measure — so the cue is worked out again when the panel is opened, and again whenever a facet is reopened from its «Cambia» link, rather than only once on load.
- **The article block on an Emeroteca page shares the page's margin.** The block that lists articles carried a width container of its own while the pages around it use the site's, so on a masthead page and on the Emeroteca home it sat eighty-six pixels further in on each side than the heading, the search box and the list of mastheads above it. The block no longer decides how wide the page is — it is placed inside the page container like everything else, and the article search page now uses the same container as the rest of the Emeroteca instead of a narrower one of its own.

### Changed
- **The privacy settings tab is organised by subject.** It used to interleave three of them: one button labelled "Salva Privacy Policy" saved the privacy page, the cookie policy page, the switch that turns the cookie banner on, two links and the category visibility — while the texts of that same banner sat in a second form with a second button, and nothing on the page said so. A librarian who enabled the banner and rewrote its wording had to press two different buttons to keep both, with no way to tell which was which. There are still two forms, but the line between them is now the subject rather than an accident of which endpoint existed: **the pages** a visitor reads at /privacy and /cookies, each with its own preview link, and **the cookie banner** — whether it appears, which categories it offers, the links in its preferences panel and everything it says — saved as one thing by one button. Turning the banner on or off, and choosing which categories it offers, is now admin-only, as its wording already was: it governs what every visitor is asked to consent to.
...
- **The catalogue search reaches the articles in the Emeroteca** ([#412](https://github.com/fabiodalez-dev/Pinakes/issues/412)). A catalogue search that matched an article used to produce a line of prose and nothing to click. The hint now carries two sections — the articles themselves and the mastheads that hold them — each entry linking to its record, with a "see all N results" line when there are more than fit. Articles gained a cover image on the cataloguing form, and the public listings gained filters by author, publication and keyword, with the active filters shown as chips that can be removed one at a time.
...
- **A saved article no longer reports "Salvataggio non riuscito".** Post-write cleanup ran inside the same guard as the write, and under this project's mysqli error mode a failure there throws rather than returning false — so the defensive branches in the cleanup helpers were dead code and the exception surfaced as a failed save over a record that had actually been stored. Worse, the rollback then deleted the PDF the committed row still referenced, and the re-rendered form carried the pre-save revision, so the retry and the delete button were both rejected as a phantom concurrent edit. The cleanup helpers can no longer propagate a failure — which also closes the same hazard at the other eighteen call sites that used them — and the rollback is gated on whether this request actually wrote the row.
- **The author link on a co-authored article led nowhere.** It filtered on the whole credit line, so "browse every article by this author" matched only articles crediting exactly the same list of names. Names are now split on `;` alone, never on the comma that belongs inside "Cognome, Nome", and each one links separately on both the listing and the detail page. The separator is documented on the form and in the plugin's README rather than left to guesswork.
- **Removing one filter chip could silently remove another.** The link was built with a filter that drops every falsy value, so a sibling filter whose value was the string `0` — an ordinary issue number for a periodical — disappeared along with the one that was clicked. One definition of "empty" now builds every address the listing produces.
- **Filtered article listings advertised themselves as indexable.** Narrowing by masthead or by search term left the page `index,follow` while pointing at the same canonical address, and pages after the first canonicalised onto page one, so records reachable only later in the sequence had nothing pointing at them. Both now follow the rule the catalogue already applied to its own search.
- **The search hint's masthead count disagreed with the page it linked to.** The hint matched mastheads on title, subtitle and ISSN while the destination also matched them through their indexed articles, so the label could say "(1)" over a page reporting two; and a term too short for the full-text index produced no hint at all while the page listed results. Both sides now build from one definition of the predicate.
- **The published sitemap outlived the plugin changes that invalidated it.** Invalidation was wired only into the cache-clearing path, which neither bundled-plugin registration nor orphan cleanup goes through — including during an upgrade, where the old file is explicitly preserved. Both now invalidate, but only when they actually changed plugin state: clearing unconditionally would delete the file on every admin page view and every maintenance window, so the fast path would cease to exist.
- **The plugin self-heal stopped repeating itself.** It read "no registered hooks" as a missed sync, which is permanently true for a plugin whose hooks depend on configuration it has not been given, so it re-ran that plugin's activation on every maintenance pass for ever. It now records that the attempt achieved nothing for this version and skips it until the version changes — while an incomplete schema still triggers a repair regardless, since suppressing a real one would re-create the missing-table failure of #138.
- **`plugin_data` could not be written twice.** The table's key pair was not unique, so the upsert the API has always used never matched and every write appended a row, leaving reads to return the value the caller thought had been replaced. `migrate_0.7.86.sql` collapses any duplicates to the newest row per key before adding the constraint.
- **A restored backup no longer stops at the first generated column.** The dump listed every column of every table and the import replayed it verbatim, but MySQL refuses an `INSERT` that supplies a value for a generated column — and this schema has one, `bookclub_member_loans.open_key`. A restore therefore failed on that table and left the database part-way through. The dump now asks `information_schema` which columns are actually insertable, names them in both the `SELECT` and the `INSERT`, and falls back to `SHOW COLUMNS` when the probe is refused.
- **An interrupted restore no longer reopens the site on a half-replaced database.** MySQL cannot roll back DDL, so an archive whose `database.sql` fails part-way through leaves some tables replaced and the rest as they were — and the code lifted maintenance mode and reported the restore as done, handing the library a database that is neither the backup nor what preceded it. The window between the first table being dropped and the import finishing is now marked: the shutdown handler leaves the site closed while it is open, and the failure comes back as a partial restore naming what happened instead of as a success. The flag it leaves behind is also exempt from the thirty-minute staleness net in the front controller — that net exists for an update that died, and a large import easily outlasts it, so without the exemption the site would have reopened itself on the half-replaced database a moment later.
- **An invitation email now carries a password link that works.** Creating a user minted a setup token and stored it, and the notifier then minted a second one for the same account before sending — so the database held the second while the email carried the first, and the recipient's only link answered "token expired" on the first click. Minting has a single owner now, `PasswordSetupToken`, which the notifier is handed rather than duplicating, and the twenty-four hours the email promises is the constant the code actually uses.
- **Resetting a forgotten password now ends the sessions someone else was holding.** The point of a reset is that whoever had the account stops having it, but remember-me sessions and mobile tokens survived it untouched, so an intruder who had already signed in stayed signed in. A reset now revokes every session on the account and every mobile application token. That includes an ordinary sign-in, which is the part that was missing: only a remembered one left a row anywhere, and everything else was decided from the PHP session, which the authentication middleware trusts without asking the database — so a reset ended the intruder's automatic sign-ins while the browser they were already using carried on. Every sign-in now records a row, whether or not it asked to be remembered. Changing the password from the profile revokes the same set but keeps the device doing the changing, and says how many sessions it closed. The write and the revocations are one act: the connection runs in autocommit, so the new password used to land on its own and a revocation that failed afterwards left the account carrying it with the old access still open — while the page reported success. A failure now rolls the whole thing back and leaves the reset token unused, so the link in the mailbox still works.
- **Signing in retires the remembered cookie the browser was already carrying.** A sign-in replaced the session but not the cookie, so on a shared machine — a reading-room terminal is the case that matters — the previous person's token stayed live in the browser and would sign it back in as them once the new session lapsed. Reaching the login form that way is not possible, because the cookie signs you in before you get there; a sign-in posted over the top of it was, and that is the path that stayed open. The cookie is now cleared and the row behind it revoked, which costs its owner only the device being handed over.
- **A revoked remember-me session is now actually revoked.** Revoking the row stopped the cookie from opening a new session, but a session already established from it carried on until it expired on its own, because nothing tied the live session to the row it came from. The session now records which row issued it, and a request whose row has been revoked, has expired or no longer exists is signed out on the spot.
- **An administrative endpoint answers a page's own JavaScript with 403 instead of the login page — and one of them was not administrative at all.** The middleware recognised an API request only by a `/api/` prefix, so an action called over XHR received a redirect the interface reported as a malformed answer rather than as "you no longer have access"; it now also recognises `X-Requested-With: XMLHttpRequest` and an `Accept` header asking for JSON. Separately, the digital library's upload route carried no administrative check at all, so a suspended account reached the handler.
- **Extending several loans at once no longer grants an extension over a reservation confirmed while the page was open.** The query that gathers the selection ran inside the transaction, and under REPEATABLE READ the first consistent read fixes the snapshot every later non-locking read in that transaction is stuck with — including the capacity checks, which run *after* the locks are taken precisely so they can see what was committed in the meantime. A reservation confirmed between the scan and the lock was invisible to them, and the extension was granted on top of it. (The loans themselves were never at risk: they are re-read under their own lock, and any that closed meanwhile are skipped.) The gathering now happens before the transaction opens, which is the order `close()`, `renew()` and `store()` already follow, and already explain in a comment.
- **A link in an email can no longer point at a host the request chose.** The address in password-recovery and invitation emails was built from the host of the request that triggered it, so a request carrying a forged `Host` produced a mail with a valid token pointing at somebody else's site. Every emailed link now comes from `TrustedLink`, which uses the configured canonical address and, when there is none, sends nothing rather than a link to an unverified host — closing the same hazard in three senders, one of which, the invitation mail, had not been flagged. The link carries the installation's base path too, so a library living in a sub-directory is not sent a valid token pointing one directory too high. An invitation that could not be sent is now reported to the administrator who created or promoted the account, instead of being logged and called a success.
...
- New suites: `tests/migration-0.7.86.unit.php` (the real migration against a sandbox seeded with the old schema: the collapse, the surviving value, the constraint and two clean re-runs) and `tests/plugin-selfheal-idempotence.unit.php` (the marker suppresses the pointless repeat, a version bump lifts it, and an incomplete schema still heals with the marker in place). `tests/sitemap-cache-invalidation.unit.php` gains a no-op guard asserting the published file *survives* a maintenance pass that changed nothing — the assertion that was missing, and which caught a real regression on its first run. `tests/emeroteca-412.unit.php` goes from 159 to 170 checks.
- The eight security and backup findings are each covered by a suite that exercises the real thing rather than the shape of the code path: `tests/backup-generated-columns.unit.php` dumps a real table carrying a generated column, replays the dump into a second table and compares, and derives the list of generated columns from `information_schema` so the next one added anywhere breaks it; `tests/backup-partial-restore.unit.php` restores a deliberately broken archive into a sandbox database and asserts the site stays closed, then that a valid archive still reopens it; `tests/auth-credential-lifetime.unit.php` covers the token contract, what a reset revokes, what a change keeps, the bound-session check and the middleware through real PSR-7 requests; `tests/loan-bulk-extend-snapshot.unit.php` puts two connections on the same sandbox and shows the snapshot the old ordering produced.
...
### Fixed
- **Backups keep the ones you asked for, and rotate the ones you did not.** The rotation added in 0.7.83 kept the ten most recent automatic backups; it now records where each archive came from — automatic, manual, pre-restore safety copy or uploaded — shows it in the list, and only ever rotates the automatic ones, so a backup taken by hand before a risky change is never reclaimed to make room. Legacy `update_*` directories rotate too, but only when a directory really is one: one holding anything besides its `database.sql` is left alone instead of being deleted with its extra files. A failed deletion is reported rather than passed over in silence.
- **A copy out of circulation no longer erases the book's holdings.** With its only copy under maintenance, a book published "Available Copies 0 / 0" ([#426](https://github.com/fabiodalez-dev/Pinakes/issues/426)), which reads as "this library does not have it". `libri.copie_totali` counts copies *in circulation* — it is the lending capacity, shared with the loan logic, the integrity checks and the edit form — so the catalogue now publishes what the library owns instead, and says why some copies are not available: "0 / 1 — Copies out of circulation: Under maintenance: 1". The same numbers reach the page, the live availability payload and `/api/books/{id}/availability`, where `copies_total` keeps its meaning and `copies_owned` is added next to it. When every owned copy is out of circulation the request button no longer offers a reservation the queue would refuse.
