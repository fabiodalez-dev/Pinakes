...
- **Extending several loans at once now sees the loans that changed while the page was open.** The query that gathers the selection ran inside the transaction, and under REPEATABLE READ the first read fixes the snapshot every later non-locking read in that transaction is stuck with — so a loan returned by somebody else a moment earlier was extended anyway. The gathering now happens before the transaction opens, which is the order `close()`, `renew()` and `store()` already follow, and already explain in a comment.
