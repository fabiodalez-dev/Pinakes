...
class Validator {
    ...
    public function validateEmail($email, $field = 'Email') {
        if (empty($email)) {
            $this->errors[$field] = "{$field} è richiesto";
            return false;
        }

        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $this->errors[$field] = "{$field} non è valido";
            return false;
        }
    ...
    public function validatePasswordConfirmation($password, $confirmation) {
        if ($password !== $confirmation) {
            $this->errors['password_confirm'] = "Le password non corrispondono";
            return false;
        }
    ...
    public function validateDatabaseConnection($host, $username, $password, $database, $port, $socket = null) {
        try {
            ...
            if (!empty($database)) {
                ...
                if (!$dbExists) {
                    $this->errors['database'] = "Database '{$database}' non esiste. Crealo prima di procedere.";
                    return false;
                }
    ...
    public function validateSystemRequirements() {
        ...
        foreach ($requirements as $req => $met) {
            if (!$met) {
                $allMet = false;
                $this->errors[$req] = "Requisito '{$req}' non soddisfatto";
            }
        }
        ...
        foreach ($directories as $dir) {
            ...
            if (!is_dir($dir)) {
                if (!mkdir($dir, 0755, true)) {
                    $this->errors['permissions'] = "Non è possibile creare la directory: {$dir}";
                    $allWritable = false;
                    continue;
                }
            }
            ...
            if (!is_writable($dir)) {
                $this->errors['permissions'] = "Directory non scrivibile: {$dir}";
                $allWritable = false;
            }
        }
    ...
    public function validateFileUpload($file, $allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'], $maxSize = 5242880) {
        if (!isset($file['error']) || is_array($file['error'])) {
            $this->errors['file'] = "Errore nel caricamento del file";
            return false;
        }
        ...
        switch ($file['error']) {
            ...
            case UPLOAD_ERR_FORM_SIZE:
                $this->errors['file'] = "Il file supera la dimensione massima consentita";
                return false;
            default:
                $this->errors['file'] = "Errore sconosciuto durante il caricamento";
                return false;
        }

        // Check file size
        if ($file['size'] > $maxSize) {
            $this->errors['file'] = "Il file supera la dimensione massima di " . ($maxSize / 1024 / 1024) . "MB";
            return false;
        }
        ...
        if (!in_array($mimeType, $allowedTypes)) {
            $this->errors['file'] = "Tipo di file non consentito. Sono ammessi solo: " . implode(', ', $allowedTypes);
            return false;
        }
