...
INSERT INTO `system_settings` (`category`, `setting_key`, `setting_value`, `description`) VALUES
('loans', 'allow_multiple_loans_same_book', '0', 'Allow the same user to have multiple active loans for the same book on distinct physical copies')
ON DUPLICATE KEY UPDATE description = VALUES(description), updated_at = NOW();
...
INSERT INTO `system_settings` (`category`, `setting_key`, `setting_value`, `description`) VALUES
('loans', 'recall_auto_enabled', '0', 'Enable automatic recalls for overdue loans')
ON DUPLICATE KEY UPDATE description = VALUES(description), updated_at = NOW();

INSERT INTO `system_settings` (`category`, `setting_key`, `setting_value`, `description`) VALUES
('loans', 'recall_interval_days', '7', 'Days between automatic recalls')
ON DUPLICATE KEY UPDATE description = VALUES(description), updated_at = NOW();

INSERT INTO `system_settings` (`category`, `setting_key`, `setting_value`, `description`) VALUES
('loans', 'recall_max_count', '3', 'Maximum automatic recalls per loan')
ON DUPLICATE KEY UPDATE description = VALUES(description), updated_at = NOW();
...
INSERT INTO `email_templates` VALUES (23,'loan_recall_notification','en_US','📢 Reminder no. {{recall_number}} - Return required','<h2>Return reminder</h2>\n<p>Hi {{user_name}},</p>\n<p>Despite previous notices, the following loan is still overdue and the book has not been returned:</p>\n<ul>\n    <li>Book: {{book_title}}</li>\n    <li>Due date: {{due_date}}</li>\n    <li>Days overdue: {{days_overdue}}</li>\n    <li>Reminder number: {{recall_number}}</li>\n</ul>\n<div style=\"background-color: #fef2f2; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #ef4444;\">\n    <p><strong>❗️ Action required</strong></p>\n    <p>Please return the book as soon as possible or contact the library. Failure to return it may result in the suspension of your account and penalties.</p>\n</div>\n<p>If you have already returned the book, please disregard this message.</p>','Reminder sent to the user to return an overdue loan (automatic or manual).',1,NULL,NULL);
INSERT INTO `email_templates` VALUES (24,'loan_receipt_email','en_US','📄 Loan receipt #{{loan_id}}','<h2>Loan receipt</h2>\n<p>Hi {{user_name}},</p>\n<p>Please find attached the PDF receipt for your loan:</p>\n<ul>\n    <li>Book: {{book_title}}</li>\n    <li>Loan date: {{loan_date}}</li>\n    <li>Due date: {{due_date}}</li>\n</ul>\n<p>Keep this receipt as a reminder of the due date.</p>\n<p>Happy reading!</p>','Email sent to the user with the loan PDF receipt attached.',1,NULL,NULL);
INSERT INTO `email_templates` VALUES (25,'reservation_awaiting_approval','en_US','📚 Copy assigned — wait for pickup confirmation','<h2>A copy has been assigned to your reservation</h2><p>Hi {{utente_nome}},</p><p>Your reservation reached the front of the queue and a copy has been allocated for {{data_inizio}} - {{data_fine}}.</p><p><strong>Book:</strong> {{libro_titolo}} — {{libro_autore}} ({{libro_isbn}})</p><div style="background-color:#fef3c7;padding:15px;border-left:4px solid #f59e0b"><p><strong>Please do not visit the library yet.</strong></p><p>The request still needs approval. We will send another email with the pickup deadline when the book is ready.</p></div><p><a href="{{book_url}}">View book</a> · <a href="{{profile_url}}">My loans</a></p>','Inviata quando una prenotazione ottiene una copia e attende l\'approvazione del prestito.',1,NULL,NULL);
INSERT INTO `email_templates` VALUES (26,'loan_copy_outcome','en_US','⚠️ Update about your borrowed copy','<h2>Loan update</h2><p>Hi {{utente_nome}},</p><p>The loan below was closed with an outcome concerning the physical copy:</p><div style="background-color:#fff7ed;padding:15px;border-left:4px solid #f59e0b"><p><strong>Book:</strong> {{libro_titolo}}</p><p><strong>Outcome:</strong> {{esito_copia}}</p><p><strong>Closed on:</strong> {{data_chiusura}}</p><p><strong>Notes:</strong> {{note}}</p><p><strong>Recorded charge:</strong> {{sanzione}}</p></div><p>Please contact the library if you have any questions.</p>','Inviata quando un prestito viene chiuso dichiarando la copia persa o danneggiata.',1,NULL,NULL);
INSERT INTO `email_templates` VALUES (27,'loan_picked_up','en_US','📖 Happy reading! Your loan has started','<h2>Pickup confirmed</h2>\n<p>Hi {{utente_nome}},</p>\n<p>You picked up the book and your loan has officially started:</p>\n<div style="background-color: #ecfdf5; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #10b981;">\n    <p><strong>Book:</strong> {{libro_titolo}}</p>\n    <p><strong>Pickup date:</strong> {{data_prestito}}</p>\n    <p><strong>Due date:</strong> {{data_scadenza}}</p>\n</div>\n<p><strong>Important:</strong> Please return the book by the due date. You will receive a reminder a few days before.</p>\n<p>Happy reading!</p>','Inviata quando il lettore ritira fisicamente il libro e il prestito inizia.',1,NULL,NULL);
