...
CREATE TABLE IF NOT EXISTS archive_activities (
    ...
    activity_type ENUM(
        'function',        -- ISDF top-level function (e.g. "Fiscal administration")
        'activity',        -- ISDF activity within a function
        'transaction',     -- ISDF granular transaction
        'task',            -- finer-grained sub-task (Pinakes extension)
        'mandate'          -- legal mandate authorising the activity
    ...
    place_id      BIGINT UNSIGNED NULL,                 -- reserved for Phase 4 archive_places FK
    source_ref    VARCHAR(500) NULL,                    -- norm / law / mandate citation
    ...
    -- Note: MySQL rejects a CHECK constraint on a column that is also
    -- part of a FK referential action (ON DELETE SET NULL).
    -- Self-parent and deeper-cycle guards therefore live in the
    -- application layer (ArchivesPlugin::activityWouldCreateCycle).
