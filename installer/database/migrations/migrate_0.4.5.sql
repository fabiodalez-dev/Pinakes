...
INSERT INTO `email_templates` (`name`, `locale`, `subject`, `body`, `description`, `active`, `created_at`, `updated_at`)
VALUES ('loan_pickup_ready', 'it_IT', '📦 Libro pronto per il ritiro!',
'<h2>Libro pronto per il ritiro!</h2>
...
<p>Buona lettura!</p>',
'Inviata quando un prestito è stato approvato e il libro è pronto per il ritiro.', 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `updated_at` = NOW();

-- Template: Pickup Expired (IT)
INSERT INTO `email_templates` (`name`, `locale`, `subject`, `body`, `description`, `active`, `created_at`, `updated_at`)
VALUES ('loan_pickup_expired', 'it_IT', '⏰ Tempo per il ritiro scaduto',
...
'Inviata quando il tempo per ritirare un libro è scaduto e il prestito è stato annullato.', 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `updated_at` = NOW();

-- Template: Pickup Cancelled (IT)
INSERT INTO `email_templates` (`name`, `locale`, `subject`, `body`, `description`, `active`, `created_at`, `updated_at`)
VALUES ('loan_pickup_cancelled', 'it_IT', '❌ Ritiro annullato',
...
<p>Cordiali saluti,<br>Il team della biblioteca</p>',
'Inviata quando un ritiro viene annullato dall''amministratore.', 1, NOW(), NOW())
ON DUPLICATE KEY UPDATE `updated_at` = NOW();
