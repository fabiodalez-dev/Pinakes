...
-- /admin/plugins immediately (is_active=0 — opt-in). ArchivesPlugin's
-- autoRegisterBundledPlugins() at request time would do this too, but
-- seeding it in the migration matches the pattern used for
-- discogs/deezer/musicbrainz in their own 0.5.4 migration window.
