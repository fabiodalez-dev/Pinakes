...
<p class="step-description">
    <?= __("Il Sistema Biblioteca è stato installato con successo e è pronto per essere utilizzato.") ?>
</p>
    ...
    <div class="alert alert-error" style="margin-top: 20px; border: 3px solid #dc2626;">
        ...
        <p style="margin-bottom: 15px;">
            <strong><?= __("Cosa fare:") ?></strong> <?= __("Devi eseguire") ?> <code style="background: #2d3748; color: #fff; padding: 3px 8px; border-radius: 4px;">composer install</code> <?= __("sul server tramite SSH.") ?>
        </p>
...
# <?= __("Output atteso: cartella vendor/ con sottocartelle (slim, monolog, etc.)") ?></pre>
                ...
                <p style="margin-top: 15px; color: #10b981;">
                    ✅ <strong><?= __("Fatto!") ?></strong> <?= __("Ora puoi ricaricare questa pagina - il warning sparirà se tutto è OK.") ?>
                </p>
            </div>
        </details>
        ...
        <div style="margin-top: 15px; padding: 15px; background: #fef3c7; border-left: 4px solid #f59e0b; border-radius: 4px;">
            <p style="margin: 0; color: #92400e;">
                ...
                <?= __("Contatta il tuo provider di hosting e chiedi di eseguire") ?> <code>composer install --no-dev</code> <?= __("nella directory dell'applicazione.") ?>
            </p>
        </div>
    </div>
    <?php else: ?>
    ...
    ?>
        <div class="alert alert-warning" style="margin-top: 20px;">
            <h4 style="margin-bottom: 10px;"><i class="fas fa-exclamation-triangle"></i> <?= __("Attenzione: Azione Manuale Richiesta") ?></h4>
            <p><?= __("L'utente del database non ha i permessi per creare i TRIGGER. L'installazione è stata completata, ma per garantire la piena integrità dei dati è necessario installarli manualmente.") ?>< non ha i permessi per creare i TRIGGER. L'installazione è stata completata, ma per garantire la piena integrità dei dati è necessario installarli manualmente.</p>
            <p style="margin-top: 10px;"><strong><?= __("Azione richiesta:") ?></strong> <?= __("Chiedi al tuo amministratore di database di eseguire i comandi contenuti nel file") ?> di database di eseguire i comandi contenuti nel file <code>installer/database/triggers.sql</code>.</p>
        </div>
    <?php endif; ?>
...
<div class="alert alert-info" style="margin-top: 30px;">
    ...
    <small style="opacity: 0.8;"><?= __("Conserva queste informazioni in un luogo sicuro!") ?> in un luogo sicuro!</small>
</div>
<?php endif; ?>
...
<ol style="list-style: decimal; margin-left: 20px; color: #4a5568;">
    <li style="margin-bottom: 10px;"><?= __("Accedi all'area admin con le credenziali sopra indicate") ?> con le credenziali sopra indicate</li>
    <li style="margin-bottom: 10px;"><?= __("Configura le impostazioni rimanenti (privacy, contatti, etc.)") ?> (privacy, contatti, etc.)</li>
    <li style="margin-bottom: 10px;"><?= __("Aggiungi scaffali e mensole per la tua biblioteca") ?> per la tua biblioteca</li>
...
<div style="margin-top: 40px; padding: 20px; background: #f7fafc; border-radius: 8px; border-left: 4px solid #16a34a;">
    ...
    <p style="color: #4a5568; margin-bottom: 15px;">
        <?= __("Per motivi di sicurezza, è") ?> <strong><?= __("altamente consigliato") ?></strong> eliminare la cartella <code>installer/</code> <?= __("dopo aver completato l'installazione.") ?>
    </p>
    <?php if (!$vendorExists): ?>
...
<div style="margin-top: 30px; text-align: center; padding: 20px; background: #f7fafc; border-radius: 8px;">
    <p style="color: #718096; margin-bottom: 10px;">
        <?= __("Grazie per aver scelto il Sistema Biblioteca!") ?>
    </p>
