...
**Key points:**
- Only plugins in `BUNDLED_PLUGINS` are overwritten during updates
- `scraping-pro` is a **premium add-on** — it is NEVER overwritten by updates
- User-installed third-party plugins are left untouched
- The same list exists in both `Updater.php` and `PluginManager.php` — keep them in sync
