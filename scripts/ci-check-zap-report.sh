...
if [[ $# -ne 3 ]]; then
    echo "Usage: $0 REPORT_JSON CATALOGUE_IDENTIFIERS_JSON PUBLIC_CATALOGUE_URLS_JSON" >&2
    exit 2
fi
...
echo "ZAP found no blocking medium/high passive-scan alerts (allowlisted: known 13-digit ISBN/EAN codes on registered bibliographic pages)"
