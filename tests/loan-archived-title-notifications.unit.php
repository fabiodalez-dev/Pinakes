...
$check(substr_count($notifSrc, "translateInLocale('Nessun motivo specificato'") >= 1
    && str_contains($notifSrc, 'function sendLoanRejectedNotificationDirect(')
    && !str_contains($notifSrc, "__('Nessun motivo specificato')"),
    '25 the reject sender localizes the fallback reason in the RECIPIENT locale');
