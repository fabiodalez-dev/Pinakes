...
$designSidecar = json_decode((string) file_get_contents($root . '/.impeccable/design.json'), true);
...
$checks = [
    ...
    'plugin actions adapt to all four public layouts' => str_contains($css, 'body.layout-editorial #book-action-buttons .plugin-book-action') && str_contains($css, 'body.layout-workspace #book-action-buttons .plugin-book-action') && str_contains($css, 'body.layout-command #book-action-buttons .plugin-book-action') && str_contains($css, 'body.layout-soft #book-action-buttons .plugin-book-action') && str_contains($css, '.plugin-source-search--frontend'),
    ...
    'design system guide and sidecar are valid project artifacts' => str_contains($designGuide, '## Overview') && str_contains($designGuide, "## Do's and Don'ts") && is_array($designSidecar) && ($designSidecar['schemaVersion'] ?? null) === 2,
    ...
    'design system documents the date selection modal' => str_contains($designGuide, '### Date Selection Modal') && is_array($designSidecar['components'] ?? null) && count(array_filter($designSidecar['components'], static fn(array $component): bool => ($component['name'] ?? '') === 'Date Selection Modal')) === 1,
    'header keeps compact search beside account actions' => str_contains($layout, 'class="mobile-search-toggle"') && str_contains($layout, 'justify-content: flex-start;'),
