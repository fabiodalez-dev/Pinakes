...
<div class="container py-4">
  ...
  <?php if ($status === 'draft' && $canManage): ?>
    ...
    <section class="bc-card">
      ...
      <p class="bc-muted text-sm mb-3"><?= $e(__('Dopo la pubblicazione le domande non saranno più modificabili.')) ?></p>
      ...
      <?php foreach ($schema as $i => $q): ?>
        <div class="border rounded-md px-3 py-3 mb-3">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <span class="bc-muted text-sm mr-2"><?= (int) $i + 1 ?>.</span>
              <span class="font-medium"><?= $e($q['label']) ?></span>
              <?php if ($q['required']): ?><span class="ml-1" style="color:var(--danger-color)">*</span><?php endif; ?>
      ...
      <!-- Add question -->
      <form method="post" action="<?= $e($base . '/questions/add') ?>" class="flex flex-wrap -mx-3 gap-y-3 mt-2 border-t pt-3">
        <input type="hidden" name="csrf_token" value="<?= $e($csrf) ?>">
        <div class="w-full px-3">
          <label class="form-label text-sm"><?= $e(__('Nuova domanda')) ?> *</label>
          <input type="text" name="label" maxlength="190" required placeholder="<?= $e(__('Es. Qual è il tuo personaggio preferito?')) ?>" class="form-input">
        </div>
        <div class="w-full px-3 md:w-1/2">
          <label class="form-label text-sm"><?= $e(__('Tipo di domanda')) ?></label>
        ...
        <div class="w-full px-3 md:w-1/2 flex items-end">
          <div class="flex items-center gap-2">
            <input class="h-4 w-4 rounded border-gray-300" type="checkbox" name="required" value="1" id="question-required">
            <label class="text-sm text-gray-700" for="question-required"><?= $e(__('Risposta obbligatoria')) ?></label>
          </div>
        </div>
        <div class="w-full px-3">
          <label class="form-label text-sm"><?= $e(__('Opzioni (una per riga, solo per scelta singola/multipla)')) ?></label>
          <textarea name="options" rows="3" class="form-input" placeholder="<?= $e(__('Una opzione per riga')) ?>"></textarea>
        </div>
        <div class="w-full px-3">
    ...
    <section class="bc-card">
      ...
      <p class="bc-muted text-sm mb-3"><?= $e(__('Titolo, libro, anonimato e date sono modificabili solo finché il questionario è una bozza.')) ?></p>
      <form method="post" action="<?= $e($base . '/update') ?>" class="flex flex-wrap -mx-3 gap-y-3">
        <input type="hidden" name="csrf_token" value="<?= $e($csrf) ?>">
        <div class="w-full px-3">
        ...
        <div class="w-full px-3 md:w-1/2 flex items-end">
          <div class="flex items-center gap-2">
            <input class="h-4 w-4 rounded border-gray-300" type="checkbox" name="anonymous" value="1" id="survey-anon" <?= $anonymous ? 'checked' : '' ?>>
            <label class="text-sm text-gray-700" for="survey-anon">
    ...
    <?php if ($status === 'open'): ?>
      <?php if ($notYetOpen): ?>
        ...
        <div class="bc-card bc-muted">
          <i class="far fa-clock mr-1" style="color:var(--primary-color)"></i>
      ...
      <?php elseif ($isMember && $myAnswer === null): ?>
        ...
        <section class="bc-card">
          ...
          <?php if ($anonymous): ?>
            <p class="bc-muted text-sm mb-3"><i class="fas fa-user-secret mr-1"></i><?= $e(__('Questionario anonimo: il tuo nome non sarà mai mostrato. La partecipazione viene registrata solo per garantire una risposta a testa.')) ?></p>
          <?php else: ?>
            <p class="bc-muted text-sm mb-3"><?= $e(__('Puoi rispondere una sola volta.')) ?></p>
          <?php endif; ?>
            ...
            <?php foreach ($schema as $i => $q): ?>
              ...
              <div class="py-3 border-b">
                ...
                <?php if ($q['type'] === 'short_text'): ?>
                  <input type="text" name="<?= $e($field) ?>" maxlength="500" <?= $q['required'] ? 'required' : '' ?> class="form-input">

                <?php elseif ($q['type'] === 'long_text'): ?>
                  <textarea name="<?= $e($field) ?>" rows="4" maxlength="5000" <?= $q['required'] ? 'required' : '' ?> class="form-input"></textarea>

                <?php elseif ($q['type'] === 'single_choice'): ?>
                  <?php foreach ($q['options'] as $oi => $option): ?>
                    <div class="flex items-center gap-2">
                      <input class="h-4 w-4 rounded border-gray-300" type="radio" name="<?= $e($field) ?>" value="<?= $e($option) ?>" id="<?= $e($field) ?>-<?= (int) $oi ?>" <?= $q['required'] ? 'required' : '' ?>>
                      <label class="text-sm text-gray-700" for="<?= $e($field) ?>-<?= (int) $oi ?>"><?= $e($option) ?></label>
                    </div>
                  <?php endforeach; ?>
                ...
                <?php elseif ($q['type'] === 'multi_choice'): ?>
                  <?php foreach ($q['options'] as $oi => $option): ?>
                    <div class="flex items-center gap-2">
                      <input class="h-4 w-4 rounded border-gray-300" type="checkbox" name="<?= $e($field) ?>[]" value="<?= $e($option) ?>" id="<?= $e($field) ?>-<?= (int) $oi ?>">
                      <label class="text-sm text-gray-700" for="<?= $e($field) ?>-<?= (int) $oi ?>"><?= $e($option) ?></label>
                    </div>
                  <?php endforeach; ?>
                  ...
                  <div class="flex items-center gap-4">
                    <?php for ($v = 1; $v <= 5; $v++): ?>
                      <label class="flex flex-col items-center gap-1">
                        <input class="h-4 w-4 rounded border-gray-300" type="radio" name="<?= $e($field) ?>" value="<?= $v ?>" <?= $q['required'] ? 'required' : '' ?>>
                        <span class="bc-muted text-sm"><?= $v ?></span>
                      </label>
                    <?php endfor; ?>
                  </div>
                ...
                <?php elseif ($q['type'] === 'yes_no'): ?>
                  <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2">
                      <input class="h-4 w-4 rounded border-gray-300" type="radio" name="<?= $e($field) ?>" value="yes" id="<?= $e($field) ?>-yes" <?= $q['required'] ? 'required' : '' ?>>
                      <label class="text-sm text-gray-700" for="<?= $e($field) ?>-yes"><?= $e(__('Sì')) ?></label>
                    </div>
                    <div class="flex items-center gap-2">
                      <input class="h-4 w-4 rounded border-gray-300" type="radio" name="<?= $e($field) ?>" value="no" id="<?= $e($field) ?>-no">
                      <label class="text-sm text-gray-700" for="<?= $e($field) ?>-no"><?= $e(__('No')) ?></label>
                    </div>
                  </div>
                <?php endif; ?>
              </div>
    ...
    <?php if ($results !== null): ?>
      ...
      <section class="bc-card">
        <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
          ...
          <span class="bc-muted text-sm"><?= $e(sprintf(__('%d risposte'), (int) $results['total'])) ?><?= $status === 'open' ? ' · ' . $e(__('In corso')) : '' ?></span>
        </div>
        ...
        <?php foreach ($results['questions'] as $i => $item): ?>
          ...
          <div class="py-3 border-b">
            <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
              <h3 class="font-medium mb-0" style="font-size:.95rem;color:var(--text-color)"><?= (int) $i + 1 ?>. <?= $e($q['label']) ?></h3>
              <span class="bc-muted text-sm"><?= $e(sprintf(__('%d risposte'), $answered)) ?></span>
            </div>

            <?php if (in_array($q['type'], ['single_choice', 'multi_choice', 'yes_no', 'scale_1_5'], true)): ?>
              <?php if ($q['type'] === 'scale_1_5' && $item['avg'] !== null): ?>
                <p class="bc-muted text-sm mb-2"><?= $e(__('Media')) ?>: <span class="font-semibold" style="color:var(--text-color)"><?= $e(number_format((float) $item['avg'], 2)) ?></span> / 5</p>
              <?php endif; ?>
              <?php foreach ($item['counts'] as $optionKey => $count): ?>
                ...
                <div class="flex items-center gap-3 mb-2">
                  <div class="bc-muted text-sm truncate shrink-0" style="width:10rem" title="<?= $e($optionLabel) ?>"><?= $e($optionLabel) ?></div>
                  <div class="bc-progress grow">
                    <span style="width: <?= number_format((int) $count / $maxCount * 100, 1, '.', '') ?>%; background: <?= $e($club['color']) ?>"></span>
                  </div>
                  <div class="text-right text-sm font-medium" style="width:2rem"><?= (int) $count ?></div>
                </div>
              <?php endforeach; ?>

            <?php else: ?>
              <?php if ($item['texts'] === []): ?>
                <p class="bc-muted text-sm mb-0"><?= $e(__('Nessuna risposta testuale.')) ?></p>
              <?php endif; ?>
              <ul class="mb-0 list-none">
                <?php foreach ($item['texts'] as $entry): ?>
                  <li class="rounded-md px-3 py-2 mb-2 text-sm" style="background:var(--light-bg)">
                    <?= nl2br($e($entry['text'])) ?>
                    <div class="bc-muted text-sm mt-1">
