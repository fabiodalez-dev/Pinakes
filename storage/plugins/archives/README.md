...
**Status: PHASE 5 — photographic items (ABA billedmarc)** (v0.9.0). On top of all phases 1-4b: (a) seven new columns on `archival_units` for image/AV items — `specific_material` (text/photograph/poster/postcard/drawing/audio/video/other), `dimensions`, `color_mode`, `photographer`, `publisher`, `collection_name`, `local_classification`; (b) additive migration `migrateImageColumns()` via `information_schema.COLUMNS` pre-check so existing installs get the columns on plugin re-activation; (c) form grows a collapsible "Materiale specifico" section (7 fields); (d) MARCXML export maps to ABA billedmarc tags (009*a+*g material, 088*a DK5, 096*a/c shelfmark, 245*e/f photographer+publisher, 245*m material text, 300*b color, 300*c dimensions); (e) import reverses the same tags. Phase 4c also landed: `collectMarcFields` now returns `tag → list-of-occurrences` so multi-400 (authority tracings) round-trip without data loss via the new `marcSubAll()` helper. Still missing: XSD validation (optional; requires shipping MARC21 Slim schema), Z39.50 server (separate plugin).
