...
**Status: PHASE 4b — round-trip-stable MARCXML I/O** (v0.8.1). On top of phases 1-4: (a) MARCXML import now ingests `<record type="Authority">` entries too (100/110 → name, 400 → tracings, 024/370/368/372/377/373/678 → ISAAR extended); (b) archival_unit import is now an UPSERT keyed on (institution_code, reference_code) so re-importing an exported file is idempotent — already-present rows report as "updated" instead of failing on the unique key; (c) authority import skips exact duplicates by (type, authorised_form) since ISAAR has no natural key. Import summary now breaks results into inserted / updated / skipped / authorities-inserted / authorities-skipped. Still missing: round-trip XSD validation, multi-400 / multi-410 expansion (collectMarcFields keeps last-write), Z39.50 server (separate plugin), photographic items (phase 5).
