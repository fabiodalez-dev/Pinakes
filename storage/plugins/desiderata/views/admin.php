...
<div class="space-y-6">
  ...
  <section><h2 class="text-xl font-semibold mb-3"><?= __('Libri richiesti') ?></h2>
  <?php if (!$books): ?><p><?= __('Non ci sono richieste aperte. Crea una scheda libro e seleziona Desiderata prima del numero di copie.') ?></p><?php endif; ?>
  ...
  <?php if (!empty($moreBooks) || $page > 1): ?>
  <nav aria-label="<?= $e(__('Pagine richieste')) ?>" class="flex gap-4 mt-3"><?php if ($page > 1): ?><a class="underline" href="<?= $e(url('/admin/desiderata') . '?page=' . ($page-1)) ?>"><?= __('Precedente') ?></a><?php endif; ?><?php if (!empty($moreBooks)): ?><a class="underline" href="<?= $e(url('/admin/desiderata') . '?page=' . ($page+1)) ?>"><?= __('Successiva') ?></a><?php endif; ?></nav>
  <?php endif; ?>
  </section>
  <section><h2 class="text-xl font-semibold mb-3"><?= __('Proposte di donazione') ?></h2>
  <?php if (!$offers): ?><p><?= __('Non sono ancora arrivate proposte.') ?></p><?php endif; ?>
  <?php foreach ($offers as $o): ?>
    <article class="py-5 border-b border-gray-200">
      ...
      <form method="post" action="<?= $e(url('/admin/desiderata/offers/' . (int)$o['id'])) ?>" class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <button name="action" value="received" class="btn btn-primary"><?= __('Libro arrivato: registra una copia') ?></button>
          <button name="action" value="rejected" class="btn btn-secondary"><?= __('Rifiuta proposta') ?></button>
        </div>
      </form>
      ...
      <form method="post" action="<?= $e(url('/admin/desiderata/offers/' . (int)$o['id'])) ?>" class="mt-3" onsubmit="return confirm(<?= $e(json_encode(__('Eliminare la proposta e i dati del donatore? L’operazione non è reversibile.'), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT)) ?>);">
        <input type="hidden" name="csrf_token" value="<?= $e($token) ?>">
        <button name="action" value="delete" class="text-sm text-red-700 underline"><?= __('Elimina proposta e dati del donatore') ?></button>
      </form>
    </article>
  <?php endforeach; ?>
  <nav aria-label="<?= $e(__('Pagine proposte')) ?>" class="flex gap-4 mt-4"><?php if ($page > 1): ?><a class="underline" href="<?= $e(url('/admin/desiderata') . '?page=' . ($page-1)) ?>"><?= __('Precedente') ?></a><?php endif; ?><?php if ($more): ?><a class="underline" href="<?= $e(url('/admin/desiderata') . '?page=' . ($page+1)) ?>"><?= __('Successiva') ?></a><?php endif; ?></nav>
  </section>
</div>
